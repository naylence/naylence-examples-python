...
if pki_dir.exists():
    ...
    pki_mapping["PKI_INTERMEDIATE_CA_CERT"] = str(
        (pki_dir / "intermediate-ca.crt").absolute()
    )
    pki_mapping["PKI_INTERMEDIATE_CA_KEY"] = str(
        (pki_dir / "intermediate-ca.key").absolute()
    )
    pki_mapping["PKI_INTERMEDIATE_CHAIN"] = str(
        (pki_dir / "intermediate-chain.crt").absolute()
    )
...
def expand_template(src_path, dst_path, mapping):
    ...
    if not src_path.exists():
        print(
            f"⚠️  Warning: Template file {src_path} not found, skipping {dst_path.name}"
        )
        return False
...
print("  - .secrets/oauth2-clients.json")
if generated_files:
